...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        Messaging.messaging().subscribe(toTopic: "Meal_Notifications")
    ...
    func application(_ application: UIApplication, continue userActivity: NSUserActivity, restorationHandler: @escaping ([UIUserActivityRestoring]?) -> Void) -> Bool {
        ...
        case UserActivityType.CheckMeals:
            if let viewController = window?.rootViewController as? BreakfastViewController {
                viewController.retrieveMessage()
            }
        default:
...
extension AppDelegate : GADRewardBasedVideoAdDelegate {
    func rewardBasedVideoAd(_ rewardBasedVideoAd: GADRewardBasedVideoAd, didRewardUserWith reward: GADAdReward) {
        ...
        let valueToSet : Int = (currentCoins ?? 0)+1
        UserDefaults.standard.set(valueToSet, forKey: reward.type)
    ...
    func rewardBasedVideoAdDidClose(_ rewardBasedVideoAd: GADRewardBasedVideoAd) {
        if let viewController = window?.rootViewController as? AdViewController {
            viewController.updateView()
        }
        GADRewardBasedVideoAd.sharedInstance().load(GADRequest(), withAdUnitID: "***s")
    }
}
